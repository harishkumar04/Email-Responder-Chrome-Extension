...
from main import cache_response, get_cached_response, get_instant_response
...
def test_optimizations():
    ...
    for email in test_emails:
        ...
        if response:
            print(
                f"✅ Pattern match ({(end-start)*1000:.1f}ms): {email[:30]}... → {response[:50]}..."
            )
        else:
    ...
    # Cache a response
    cache_response(
        test_email, response_type, "I'd be happy to help you with your account setup!"
    )
